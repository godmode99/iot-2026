...
| Sensor Layer       | GPS L76K/NEO-M8N           | UART → GPIO13/14                                                      | บน enclosure รับสัญญาณดาวเทียม |
| Power Layer        | 18650 x2 parallel          | → TP4056 (charge) + DW01 (protect)                                    | 6,800 mAh, ~16 วัน runtime     |
...
| Connectivity Layer | SIM7670E 4G LTE            | UART → GPIO17/18                                                      | ส่งข้อมูล MQTT over TLS        |
...
- รองรับ BLE 5.0 — ใช้สำหรับ provisioning/config ในอนาคต
- รองรับ OneWire, ADC, UART ครบ
- ราคาใกล้เคียง ESP32 รุ่นเดิม

## 2.2 4G Module — SIMCom SIM7670E / A7670C

> **หมายเหตุ:** ชื่อโมดูล: FS-HCore dev board ใช้ SIM7670C (band เดียวกัน) — PCB custom ใช้ A7670E หรือ SIM7670E แล้วแต่หาซื้อได้ — firmware AT command เหมือนกันทุกตัว ใช้แทนกันได้
...
> **หมายเหตุ:** Phase 1 ใช้ FS-HCore-A7670C (SIM7670C) — Phase 2+ ใช้ A7670E หรือ SIM7670E บน PCB custom — AT command เหมือนกัน ไม่ต้องแก้ firmware
...
| Battery                    | Capacity  | Runtime (realistic) | ราคา    |
| -------------------------- | --------- | ------------------- | ------- |
| 18650 Li-ion x1            | 3,400 mAh | ~8 วัน              | 150 THB |
| 18650 Li-ion x2 (parallel) | 6,800 mAh | ~16 วัน ★           | 300 THB |
| LiFePO4 26650 x2           | 6,400 mAh | ~15 วัน             | 500 THB |
...
| Runtime realistic                | ~7.2 วัน              | 5,600 / 775                  |
| Runtime optimistic (sleep เพิ่ม) | ~14-16 วัน            | ถ้า PSM mode ทำงานดี         |

> **หมายเหตุ:** แนะนำ: 18650 x2 parallel — runtime ~16 วัน, ชาร์จผ่าน USB-C (TP4056), มี fuel gauge (MAX17048) แจ้งระดับแบตผ่าน dashboard
...
| รายการ        | Spec                                                                                                          |
| ------------- | ------------------------------------------------------------------------------------------------------------- |
| IP Rating     | IP67 (กันน้ำจุ่มชั่วคราว 1 เมตร 30 นาที)                                                                      |
| Material      | ABS+PC UV resistant (ทนแดด)                                                                                   |
| ขนาดประมาณ    | 150 × 100 × 60 mm                                                                                             |
...
| 5 นาที (default) | ใช้งานปกติ     | ~16 วัน             |
...
## 15.2 Phase 2 — BLE Provisioning

- เปิดเครื่องครั้งแรก device เข้า BLE Provisioning Mode (ESP32-S3 BLE 5.0)
- ผู้ใช้เปิด AquaSense app Scan BLE พบ device
- ส่ง config: Device name, Farm ID, MQTT credentials, APN settings
- Device บันทึก config ลง NVS reboot เข้า normal mode
- Device register ตัวเองกับ server ผ่าน MQTT
...
| 4G Modem      | SIMCom A7670E                                       | ESP32-S3            | UART1 (GPIO17/18)                            |
| GPS           | L76K / NEO-M8N                                      | ESP32-S3            | UART2 (GPIO13/14)                            |
...
| Spec             | ค่า                                         | หมายเหตุ                                           |
| ---------------- | ------------------------------------------- | -------------------------------------------------- |
| ขนาดภายนอก       | TBD — confirm หลัง PCB layout เสร็จ Phase 2 | ประมาณ 150×100×60mm — อาจเล็กลงได้                 |
...
| การกันน้ำ        | IP67                                        | จุ่มได้ 1 เมตร 30 นาที                             |
...
| ---------------------------------- | ------------------- | ------------------ | ----------------------------- | --------------- |
| SIM7670E / A7670E                  | LTE Cat 1 4G module | LCC68 / Stamp hole | LCSC C5198236 หรือ AliExpress | ~180 THB        |
| GPS L76K                           | GNSS module UART    | Stamp hole         | LCSC C5284605 หรือ AliExpress | ~150 THB        |
| GPS NEO-M8N (ถ้าต้องการแม่นยำกว่า) | u-blox GNSS         | Module             | AliExpress                    | ~400-500 THB    |

> **หมายเหตุ:** SIM7670E และ GPS module สั่งแยกจาก LCSC part ข้างต้น หรือ AliExpress — ราคาถูกกว่าเมื่อสั่ง 10+ ชิ้น
