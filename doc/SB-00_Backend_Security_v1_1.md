...
| LINE OA                | LINE Official Account (AquaSense)   | ช่องทางรับ notification + chatbot support เบื้องต้น                                  |
...
| Certification Lab            | ห้องแล็บที่ได้รับรอง กสทช. ในไทย       | IP67 + EMC/RF testing — ยื่นเมื่อพร้อม ไม่บล็อก launch                              |
...
> **หมายเหตุ:** ทุก service มี free tier เพียงพอสำหรับเริ่มต้น ยกเว้น Upstash QStash ที่ต้อง upgrade ตั้งแต่วันแรก (~$4/mo) เพราะ free tier รองรับแค่ 500 msg/day แต่แม้แต่ 1 device ก็ส่ง 288 msg/day
...
| Upstash QStash     | 500 msg/day            | ตั้งแต่ 1 device        | Pay-per-use ~$4/mo |
| Vercel             | Hobby 100GB bandwidth  | ≥ 50 devices            | Pro $20/mo         |
| Resend             | 3,000 emails/mo        | ≥ 88 devices            | Starter $20/mo     |
| LINE Messaging API | 200 msg/mo             | ≥ 40 devices            | Light ~150 THB/mo  |
...
| ---------- | ------------------ | --------------------- | ----------------------------------------------------- |
| 1 (วันแรก) | Upstash QStash     | 1+ devices            | Free tier 500 msg/day ไม่พอแม้ 1 device (288 msg/day) |
...
| /api/ota/push       | POST   | SVC  | push OTA ไปยัง device — body: {device_id, release_id}            |
...
| Route            | การแก้ไข                                                                                                      |
| ---------------- | ------------------------------------------------------------------------------------------------------------- |
| /api/export/jobs | เพิ่ม GET route — list export jobs ทั้งหมดของ user พร้อม status + download URL                                |
...
| Deliver         | ส่งผ่าน BLE provisioning (encrypted) หรือ flash ตอนผลิตด้วย USB — ห้ามส่งผ่าน HTTP plaintext        |
...
| Vercel             | Pro                            | $20                     |
| LINE Messaging API | Light plan                     | ~$4 (~150 THB)          |
...
| ------------------ | ----------------------------------------- | ---------------------------- |
| LINE Messaging API | ส่ง push message ไป LINE OA ของ AquaSense | ~150 THB/เดือน (Light plan)  |
...
ทดสอบ 3-5 เครื่อง ในบ่อจริง 1 เดือน ก่อน commercial launch

| หัวข้อทดสอบ                 | ระยะเวลา  | สิ่งที่ต้องตรวจ                       |
| --------------------------- | --------- | ------------------------------------- |
| ความทนทาน outdoor           | 1 เดือน   | enclosure ไม่รั่ว, PCB ไม่มีน้ำเข้า   |
| Battery runtime จริง        | 1 เดือน   | เปรียบเทียบกับที่คำนวณ (~16 วัน)      |
